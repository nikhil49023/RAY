...
def default_settings() -> Dict[str, Any]:
    return {
        ...
        "agentRuntime": {
            ...
            "codexBaseUrl": os.getenv(
                "CODEX_BASE_URL", "https://api.groq.com/openai/v1"
            ),
...
def apply_runtime_settings(settings: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    if api_keys.get("huggingface"):
        os.environ["HUGGINGFACE_API_TOKEN"] = str(api_keys["huggingface"])
    if api_keys.get("firecrawl"):
        os.environ["FIRECRAWL_API_KEY"] = str(api_keys["firecrawl"])
    ...
    os.environ["RAY_SEARCH_PROVIDER"] = str(
        settings.get("search", {}).get("provider", "duckduckgo")
    )
    os.environ["RAY_RERANKER"] = str(settings.get("search", {}).get("reranker", "ReLU"))
    os.environ["RAY_RESEARCH_OUTPUT"] = str(
        settings.get("ui", {}).get("researchOutput", "document")
    )
    os.environ["RAY_SHOW_THINKING_LOGS"] = (
        "1" if settings.get("ui", {}).get("showThinkingLogs", True) else "0"
    )
    ...
    os.environ["CODEX_BASE_URL"] = str(
        runtime.get("codexBaseUrl", "https://api.groq.com/openai/v1")
    ).rstrip("/")
    os.environ["CODEX_SANDBOX"] = str(runtime.get("codexSandbox", "workspace-write"))
    os.environ["CODEX_APPROVAL_POLICY"] = str(
        runtime.get("codexApprovalPolicy", "never")
    )
...
def get_firecrawl_config(settings: Dict[str, Any] | None = None) -> Dict[str, str]:
    ...
    fallback_key = str(firecrawl.get("fallbackApiKey") or "").strip()
    strategy = (
        str(firecrawl.get("strategy") or "self_hosted_first").strip()
        or "self_hosted_first"
    )
    return {
        "strategy": strategy,
        "base_url": base_url,
...
def get_agent_runtime_config(settings: Dict[str, Any] | None = None) -> Dict[str, str]:
    ...
    return {
        ...
        "codex_model": str(runtime.get("codexModel") or "openai/gpt-oss-20b").strip()
        or "openai/gpt-oss-20b",
        "codex_provider_id": str(runtime.get("codexProviderId") or "groq").strip()
        or "groq",
        "codex_base_url": str(
            runtime.get("codexBaseUrl") or "https://api.groq.com/openai/v1"
        )
        .strip()
        .rstrip("/")
        or "https://api.groq.com/openai/v1",
        "codex_sandbox": str(runtime.get("codexSandbox") or "workspace-write").strip()
        or "workspace-write",
        "codex_approval_policy": str(
            runtime.get("codexApprovalPolicy") or "never"
        ).strip()
        or "never",
    }
